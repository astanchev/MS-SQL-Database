...
SELECT CONCAT(c.FirstName, ' ', c.LastName) Client, 
       DATEDIFF(DAY, IssueDate, '24 April 2017') [Days going], 
       j.[Status]
FROM Clients AS c
...
SELECT CONCAT(m.FirstName, ' ', m.LastName) Mechanic,
       CEILING(AVG(DATEDIFF(DAY, j.IssueDate, j.FinishDate))) [Average Days]
FROM Mechanics AS m
     JOIN Jobs AS j ON j.MechanicId = m.MechanicId
...
--10.	Hard Earners
SELECT TOP (3) CONCAT(m.FirstName, ' ' + m.LastName) Mechanic, 
               COUNT(j.JobId) Jobs
FROM Mechanics AS m
     JOIN Jobs AS j ON j.MechanicId = m.MechanicId
WHERE j.[Status] != 'Finished'
...
FROM CTE_WeeksAndPrices
WHERE Weeks <= 3;
GO

--13.	Past Expenses
SELECT j.JobId,
       ISNULL(SUM(p.Price * op.Quantity), 0.00) Total
FROM Jobs AS j
...
FROM Parts AS p
     ...
     LEFT JOIN Orders AS o ON o.JobId = j.JobId
WHERE pn.Quantity > ISNULL(p.StockQty + 
							CASE
                               WHEN o.Delivered = 0
                               THEN op.Quantity
                               ELSE 0
...
AS
     BEGIN
         IF((SELECT Delivered FROM deleted) = 0
				AND (SELECT Delivered FROM inserted) = 1)
             BEGIN
